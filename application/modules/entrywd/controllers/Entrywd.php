...
class Entrywd extends CI_Controller
{
	...
	{
		// var_dump($this->input->post('cond'));die();
		$condition = $this->db->get_where('condition', ['status' => 1, 'devisi_id' => 1])->result();
		...
		$action = $this->db->get_where('action', ['status' => 1, 'devisi_id' => 1])->result();
